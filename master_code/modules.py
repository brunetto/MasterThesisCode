...
def hdf5_data(path, filename, n_sel=None, mass_bin=None):
    ...
    if n_sel != None and mass_bin != None:
        mlogger.info("Both n_sel and mass_bin are not None, choice not yet implemented, exit...")
        sys.exit()
        
    h5f = tb.openFile(filename, mode = "r")
    if n_sel == 0 or n_sel == None:
        if mass_bin != None:
	    mlogger.info("Selected mass bin %s", mass_bin)
	    selection = h5f.getNode(h5f.root.mass_bin_indexes, mass_bin).read()
	    pos = h5f.root.data.read()[selection]
	else:
