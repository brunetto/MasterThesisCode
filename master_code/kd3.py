...
def minkowski_distance_p_c(x,y,p=2):
    """Inline C function to calculate the distances.
    """
    n1 = x.shape[0]
    n2 = y.shape[0]
    x1 = x[:,0]
	...
	for(i=0; i<n1; i++){
	  ...
	  dx = x2(j) - x1(i);
       ...
       """
    weave.inline(code, 
                 ['x1', 'y1', 'z1', 'x2', 'y2', 'z2', 'n1', 'n2', 'result'], 
...
class Rectangle(object):
    ...
    def min_distance_rectangle_f(self, other, strategy, p=2.):
        """Compute the minimum distance between points in the two hyperrectangles."""
        return minkowski_distance_p_c(np.zeros(3)[np.newaxis,:], np.maximum(0,np.maximum(self.mins-other.maxes,other.mins-self.maxes))[np.newaxis,:],p)

    def max_distance_rectangle_f(self, other, strategy, p=2.):
        """Compute the maximum distance between points in the two hyperrectangles."""
        return minkowski_distance_p_c(np.zeros(3)[np.newaxis,:], np.maximum(self.maxes-other.mins,other.maxes-self.mins)[np.newaxis,:],p)
...
#                                                p).ravel()
			# Fortran distance
                        ...
                        # Inline C distance
                        ds = minkowski_distance_p_c(self.data[node1.idx], other.data[node2.idx]).ravel()
