...
start_slice = 220
end_slice = 320    # Number of data slices

def starter(input, output):
    """Start the code processes one after the other"""
    while input.qsize() != 0:
        item = input.get()
        file_1 = "mill2sort-"+str(item[0])       #"gif2_"+str(item[0])+".h5"
        cmd = "/opt/epd-7.0-2-rh5-x86_64/bin/python -u ../serial.py --file_1 "+file_1+" --file_2 "+file_2+\
